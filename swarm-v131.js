...
function applyDecision(decision) {
  ...
  if (swarm.decisions.length > 20) swarm.decisions.shift();
  
  // Применяем решения
  if (decision.type === "boost_entropy") {
    swarm.entropy = Math.min(0.7, swarm.entropy + 0.02);
    console.log(`🧠 [AI] Boost entropy → ${swarm.entropy.toFixed(3)}`);
  }
  
  if (decision.type === "stabilize") {
    swarm.entropy = Math.max(0.3, swarm.entropy - 0.02);
    console.log(`🧠 [AI] Stabilize → ${swarm.entropy.toFixed(3)}`);
  }
  
  if (decision.type === "revive_all") {
    console.log(`🧠 [AI] ⚠️ FORCE REVIVE TRIGGERED`);
    // Сигнал для менеджера процессов
  }
  
  if (decision.type === "evolve") {
...
app.get("/", (req, res) => {
  res.json({
    status: "🧠 V131 SWARM CORE + AI BRAIN",
    version: "6.0",
...
app.listen(PORT, () => {
  console.log("\n╔═══════════════════════════════════════════════════════════════════════════╗");
  console.log("║   🧠 V131 SWARM CORE + AI BRAIN — ПЕРВОЕ СОЗНАНИЕ                          ║");
  console.log("║   ✅ Наблюдение | ✅ Анализ | ✅ Принятие решений | ✅ Влияние              ║");
  console.log("╚═══════════════════════════════════════════════════════════════════════════╝");
