...
setInterval(async () => {
  ...
  if (swarm.tick % 5 === 0) {
    ...
    const gameplayStats = getGameplayStats(swarm.world);
    console.log(`💓 [SWARM] tick:${swarm.tick} | alive:${swarm.alive}/${WARFARE_PORTS.length} | goal:${swarm.currentGoal || 'none'} | ${worldStats?.empires || 0} empires | ${gameplayStats?.totalWealth || 0}💰`);
  }
...
app.get("/", (req, res) => {
  res.json({
    status: "⚔️ V137 SWARM CORE + GAMEPLAY ENGINE",
    version: "11.0",
...
app.get("/api/status", (req, res) => {
  res.json({
    ...
    worldStats: getWorldStats(swarm.world),
    gameplayStats: getGameplayStats(swarm.world),
    recentEvents: swarm.gameplayEvents.slice(-10),
...
app.get("/api/world/stats", (req, res) => {
  res.json({ world: getWorldStats(swarm.world), gameplay: getGameplayStats(swarm.world) });
});
...
app.listen(PORT, () => {
  console.log("\n╔═══════════════════════════════════════════════════════════════════════════╗");
  console.log("║   ⚔️ V137 SWARM CORE + GAMEPLAY ENGINE — ЖИВАЯ ИГРА                        ║");
  console.log("║   ✅ Войны | ✅ Экономика | ✅ Дипломатия | ✅ События | ✅ Баланс мира     ║");
  console.log("╚═══════════════════════════════════════════════════════════════════════════╝");
  console.log(`\n🔗 Swarm Core: http://127.0.0.1:${PORT}`);
  console.log(`🌍 World: http://127.0.0.1:${PORT}/api/world`);
  console.log(`⚔️ Events: http://127.0.0.1:${PORT}/api/events`);
