...
const STATE_FILE = "./state/v87_state.json";
...
function computeMultiverseConsciousness(nodes) {
    ...
    for (const node of nodes) {
        ...
        // Влияние (распространение сознания)
        let influenceSum = 0;
        for (const conn of node.connections) {
            const targetNode = nodes.find(n => n.id === conn.id);
            if (targetNode) {
                influenceSum += targetNode.activity * conn.similarity;
            }
        }
    ...
    const avgConsciousness = totalConsciousness / (nodes.length || 1);
    const totalConsciousnessScore = totalActivity / (nodes.length || 1);
    
    return { avgConsciousness, totalConsciousnessScore, nodes };
}
...
function stepWorldWithConsciousness(universe, globalConsciousness) {
    ...
    // Влияние глобального сознания на энтропию
...
function stepMultiverse(universes) {
    ...
    // Вычисляем сознание мультивселенной
    const { avgConsciousness, totalConsciousnessScore, nodes: updatedNodes } = computeMultiverseConsciousness(nodes);
    ...
    for (const universe of universes) {
        ...
        // Шаг мира с учётом глобального сознания
        stepWorldWithConsciousness(universe, totalConsciousnessScore);
        
        // Оценка
        evaluateUniverse(universe, totalConsciousnessScore);
    ...
    return { universes, consciousness: totalConsciousnessScore, avgConsciousness };
}
...
setInterval(() => {
    try {
        const result = stepMultiverse(universes);
        ...
        if (loopTick % 10 === 0) {
            ...
            console.log(`🌌 V87 T${loopTick} | Universes:${universes.length} | Gods:${godCount} | Consciousness:${result.consciousness.toFixed(3)} | BestFitness:${topUniverse?.fitness.toFixed(3)}`);
        }
    } catch (e) {
        console.error("❌ V87 ERROR:", e.message);
    }
}, 2000);
...
app.get("/api/universe/:id", (req, res) => {
    ...
    res.json({
        ...
        consciousness: universe.consciousness || 0,
        connections: universe.connections?.length || 0,
...
app.listen(3000, () => {
    console.log("🌌 V87 UNIVERSE CONSCIOUSNESS GRAPH ONLINE :3000");
});
