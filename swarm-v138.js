...
setInterval(async () => {
  ...
  if (swarm.tick % 5 === 0) {
    ...
    const governmentStats = getGovernmentStats(swarm.world);
    console.log(`💓 [SWARM] tick:${swarm.tick} | alive:${swarm.alive}/${WARFARE_PORTS.length} | goal:${swarm.currentGoal || 'none'} | ${worldStats?.empires || 0} empires | leader:${governmentStats?.leader || 'none'}`);
  }
...
app.get("/", (req, res) => {
  res.json({
    status: "🧠⚖️ V138 SWARM CORE + GOVERNMENT SYSTEM",
    version: "12.0",
...
app.listen(PORT, () => {
  console.log("\n╔═══════════════════════════════════════════════════════════════════════════╗");
  console.log("║   🧠⚖️ V138 SWARM CORE + GOVERNMENT SYSTEM — ПОЛИТИКА МИРА                 ║");
  console.log("║   ✅ Правительства | ✅ Лидеры | ✅ Дипломатия | ✅ Стабильность           ║");
  console.log("║   ✅ Империи принимают решения | ✅ Самоуправление                         ║");
  console.log("╚═══════════════════════════════════════════════════════════════════════════╝");
