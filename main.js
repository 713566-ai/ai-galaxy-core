// Специальная версия для Replit (с поддержкой портов)
...
// Replit использует порт из переменной окружения
...
// Cloud Orchestrator (виртуальные ноды для Replit)
...
app.listen(port, '0.0.0.0', () => {
  ...
  console.log('╚════════════════════════════════════════╝');
  console.log('');
  console.log('📡 Публичный URL: https://' + process.env.REPL_SLUG + '.' + process.env.REPL_OWNER + '.repl.co');
});
