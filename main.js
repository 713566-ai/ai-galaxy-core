...
let bot = null;
let notifyNewPayment = async () => {};
let notifyServerStart = async () => {};
let notifyError = async () => {};

try {
  const tg = require('./telegram-bot.js');
  bot = tg.bot;
  notifyNewPayment = tg.notifyNewPayment;
  notifyServerStart = tg.notifyServerStart;
  notifyError = tg.notifyError;
} catch(e) {
...
console.log(`
╔═══════════════════════════════════════════════════════════════════════════════╗
║   🚀 AI GALAXY CORE v6.0 - ULTIMATE EDITION                                ║
║   🤖 500 Agents | 📈 Market | 💰 Платежи | ⚡ WebSocket | 🔒 JWT             ║
║   🧠 Smart Agents | 🌍 Global Events | 📊 Analytics | ⚡ Cached              ║
╚═══════════════════════════════════════════════════════════════════════════════╝
`);
...
const GLOBAL_EVENTS = [
  {
    ...
    probability: 0.03,
    effect: (market, agents) => {
      for (let res in market.prices) {
        market.prices[res] *= 0.6;
      }
  ...
  },
  {
    ...
    effect: (market, agents) => {
      ...
      agents.forEach(a => {
        if (Math.random() < 0.1) a.techs.push('new_tech');
      });
  ...
  },
  {
    ...
    effect: (market, agents) => {
      ...
      agents.forEach(a => {
        if (Math.random() < 0.05) a.energy *= 0.7;
      });
      console.log("🦠 Пандемия! Медицина стала дороже");
    }
  },
  {
    name: "⚔️ Межфракционная война",
    probability: 0.02,
    effect: (market, agents) => {
  ...
  },
  {
    ...
    effect: (market, agents) => {
      for (let res in market.prices) {
        market.prices[res] *= 1.2;
      }
      agents.forEach(a => {
        if (Math.random() < 0.2) a.wealth *= 1.1;
      });
  ...
  },
  {
    ...
    probability: 0.02,
    effect: (market, agents) => {
      market.prices.food *= 1.5;
      market.prices.energy *= 1.3;
      agents.forEach(a => {
        if (Math.random() < 0.03) a.isAlive = false;
      });
      console.log("🌋 Природный катаклизм! Ресурсы подорожали");
    }
  },
  {
    name: "🛸 Первый контакт",
    probability: 0.01,
    effect: (market, agents) => {
      market.prices.tech *= 2;
      market.prices.research *= 1.8;
      console.log("🛸 Первый контакт с инопланетной цивилизацией!");
    }
  }
];
...
const BANK_DETAILS = {
  ...
  swift: {
    iban: 'UA133220010000026205316571982',
    swift: 'UNJSUAUKXXX',
    name: 'KADUKOV HLIB',
    address: '50057, Ukraine, Dnipropetrovska, Kryvyi Rih, Hirnytskyi, build 34, fl. 70'
  },
  phone: '+380987979381'
};
...
class Agent {
  constructor(id, fromSave = false) {
    ...
    if (!fromSave) {
      ...
      this.inventory = {
        energy: 100, minerals: 50, food: 100, tech: 10, crystals: 5,
        weapons: 5, ships: 1, research: 2, medicine: 10, luxury: 3
      };
      ...
      // Инициализация полей при загрузке из сохранения
      if (!this.personality) {
        this.personality = {
          aggressiveness: Math.random(),
          greed: Math.random(),
      ...
      if (!this.skills) this.skills = { mining: 1, trading: 1, combat: 1, research: 1 };
      if (!this.inventory) {
        this.inventory = {
          energy: 100, minerals: 50, food: 100, tech: 10, crystals: 5,
          weapons: 5, ships: 1, research: 2, medicine: 10, luxury: 3
  ...
  updateEmotions() {
    ...
    if (this.wealth > (this.lastWealth || this.wealth)) {
      this.emotions.happiness = Math.min(100, this.emotions.happiness + 10);
      this.emotions.fear = Math.max(0, this.emotions.fear - 5);
    } else {
      this.emotions.fear = Math.min(100, this.emotions.fear + 10);
      this.emotions.happiness = Math.max(0, this.emotions.happiness - 5);
    }
    
    if (this.emotions.anger > 70) this.role = 'warrior';
    if (this.emotions.happiness > 80) this.role = 'trader';
    if (this.emotions.fear > 70) this.role = 'collector';
    
    this.lastWealth = this.wealth;
  }
  ...
  act() {
    ...
    if (this.age > 100 && Math.random() < 0.003) {
      this.isAlive = false;
    }
  }
  ...
  executeAction(action) {
    switch(action) {
      ...
      case 'collect':
        this.energy += 10;
        this.inventory.energy += Math.floor(Math.random() * 15) + 5;
        break;
      ...
      case 'research':
        ...
        if (this.experience > 100 && this.techs.length < 10) {
          ...
          this.experience = 0;
          console.log(`🔬 Agent ${this.id} discovered new technology!`);
        }
        break;
      case 'fight':
        if (Math.random() < 0.5) {
          this.wealth += 40;
          if (this.emotions) this.emotions.anger += 10;
        } else {
          this.energy -= 15;
          if (this.emotions) this.emotions.fear += 20;
        }
        break;
...
let step = 0;
let history = {
  population: [], avgWealth: [], market: [], roles: {}, generations: [], gini: []
};
...
function saveWorld(slot = 'auto') {
  try {
    ...
    const saveData = {
      version: '6.0', timestamp: Date.now(), generation, step,
      agents: aliveAgents.slice(0, 500).map(a => a.toJSON()),
      market: market.prices,
      history: {
        population: history.population.slice(-50),
        avgWealth: history.avgWealth.slice(-50),
        gini: history.gini.slice(-50)
      }
    };
...
setInterval(() => {
  ...
  batchProcessor.processAgents(agents, async (agent) => {
    agent.act();
  });
...
app.post('/api/create-payment', strictLimiter, express.json(), (req, res) => {
  ...
  const instructions = {
    ...
    swift: `🏦 SWIFT: ${BANK_DETAILS.swift.iban}\nСумма: $${prices[plan]}`
  };
