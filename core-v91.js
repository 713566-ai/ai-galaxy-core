...
const STATE_FILE = "./state/v91_state.json";
...
function computeSystemStability(multiverse) {
    let entropySum = 0;
    let growthRate = 0;
    let fitnessSum = 0;
    
    for (const u of multiverse) {
        entropySum += u.state.world.entropy;
        growthRate += u.children.length;
        fitnessSum += u.fitness;
    }
    
    return {
        entropy: entropySum / (multiverse.length || 1),
        growth: growthRate,
        avgFitness: fitnessSum / (multiverse.length || 1),
        universesCount: multiverse.length
    };
}
...
function dampRecursion(value) {
    return value / (1 + Math.log(1 + value));
}
...
function applyStabilityBrake(system) {
    const stability = system.stability;
    let changed = false;
    
    if (stability.growth > 12 && stability.entropy > 0.75) {
        const oldCount = system.multiverse.length;
        system.multiverse = system.multiverse.slice(0, Math.floor(system.multiverse.length * 0.6));
        console.log(`🛑 STABILITY BRAKE: Universes ${oldCount} → ${system.multiverse.length}`);
        changed = true;
    }
    
    if (stability.universesCount > 18) {
        const oldCount = system.multiverse.length;
        system.multiverse.sort((a, b) => b.fitness - a.fitness);
        system.multiverse = system.multiverse.slice(0, 12);
        console.log(`🛑 STABILITY BRAKE: Pruned from ${oldCount} to 12`);
        changed = true;
    }
    
    if (stability.entropy > 0.85 || stability.avgFitness < 0.2) {
        for (const u of system.multiverse) {
            u.state.world.entropy *= 0.92;
            u.state.world.entropy = Math.max(0.1, u.state.world.entropy);
            if (u.physics) {
                u.physics.mutationRate *= 0.85;
                u.physics.mutationPressure *= 0.9;
            }
        }
        console.log(`🛑 STABILITY BRAKE: Global stabilization`);
        changed = true;
    }
...
function deleteUniverse(nodes, node, reason) {
    ...
    console.log(`💀 DELETION: ${u.id} | Reason: ${reason}`);
...
function stepMultiverse(universes) {
    ...
    for (const node of toDelete) {
        const reasons = [];
        if (node.universe.fitness < 0.2) reasons.push("low_fitness");
        if (node.activity < totalConsciousness * 0.3) reasons.push("low_activity");
        if (node.universe.state.world.entropy > 0.85) reasons.push("high_entropy");
        deleteUniverse(updatedNodes, node, reasons.join(","));
    }
    
    let remainingUniverses = updatedNodes.map(n => n.universe);
...
setInterval(() => {
    try {
        ...
        const stability = computeSystemStability(universes);
        const system = { multiverse: universes, stability: stability };
        const braked = applyStabilityBrake(system);
        universes = system.multiverse;
        
        if (stability.growth > 15) {
            for (const u of universes) {
                u.physics.mutationPressure = dampRecursion(u.physics.mutationPressure);
                u.physics.warProbability = dampRecursion(u.physics.warProbability);
            }
            console.log(`🔄 DAMPING: Growth=${stability.growth}`);
        }
        
        if (loopTick % 10 === 0) {
            const topUniverse = [...universes].sort((a, b) => b.fitness - a.fitness)[0];
            const godCount = universes.filter(u => u.godAgent).length;
            console.log(`💀 V91 T${loopTick} | Universes:${universes.length} | Gods:${godCount} | Growth:${stability.growth} | Entropy:${stability.entropy.toFixed(3)}`);
        }
    } catch (e) {
        console.error("❌ V91 ERROR:", e.message);
    }
}, 2000);
...
app.get("/api/status", (req, res) => {
    ...
    res.json({
        ...
        stability: {
            entropy: stability.entropy.toFixed(3),
            growth: stability.growth,
            avgFitness: stability.avgFitness.toFixed(3)
        },
...
app.listen(3000, () => {
    console.log("💀 V91 INFINITE RECURSION COLLAPSE FIX ONLINE :3000");
});
