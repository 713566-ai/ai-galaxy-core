...
const { updateGoals, getActiveGoal, getAllGoals, getGoalStats } = require("./goals-v133");
...
setInterval(async () => {
  ...
  if (activeGoal) {
    ...
    // Логируем цель каждые 10 тиков
    if (swarm.tick % 10 === 0) {
      console.log(`🎯 [GOAL] Active: ${activeGoal.type} | Progress: ${(activeGoal.progress * 100).toFixed(1)}%`);
    }
  }
  ...
  if (swarm.tick % 5 === 0) {
    console.log(`💓 [SWARM] tick:${swarm.tick} | alive:${swarm.alive}/${WARFARE_PORTS.length} | goal:${swarm.currentGoal || 'none'}`);
  }
...
app.get("/", (req, res) => {
  res.json({
    status: "🎯 V133 SWARM CORE + GOALS SYSTEM",
    version: "8.0",
...
app.post("/api/goals/create", (req, res) => {
  ...
  const goal = require("./goals-v133").createGoal(type, priority, target);
...
app.listen(PORT, () => {
  console.log("\n╔═══════════════════════════════════════════════════════════════════════════╗");
  console.log("║   🎯 V133 SWARM CORE + GOALS SYSTEM — СИСТЕМА ЦЕЛЕЙ                       ║");
  console.log("║   ✅ Авто-цели | ✅ Приоритеты | ✅ Прогресс | ✅ Намерение                ║");
  console.log("╚═══════════════════════════════════════════════════════════════════════════╝");
  ...
  console.log(`📊 Stats: http://127.0.0.1:${PORT}/api/goals/stats`);
